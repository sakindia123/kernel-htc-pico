/* linux/arch/arm/mach-msm/board-pico-mmc.c
...
static void __init msm7x27a_init_mmc(void)
{
	...
	/* SDIO WLAN slot */
    /* SDCC : WLAN chip is connected; we do in pico_init_mmc*/
#ifdef CONFIG_MMC_MSM_SDC2_SUPPORT
...
static uint32_t wifi_on_gpio_table[] = {
	...
	GPIO_CFG(PICO_GPIO_WIFI_IRQ, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA), /* WLAN IRQ */
};
...
static uint32_t wifi_off_gpio_table[] = {
	...
	GPIO_CFG(PICO_GPIO_WIFI_IRQ, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_4MA), /* WLAN IRQ */
};
 ...
 */
static struct embedded_sdio_data pico_wifi_emb_data = {
	.cccr	= {
		.sdio_vsn	= 2,
		.multi_block	= 1,
		.low_speed	= 0,
...
static int
pico_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static int pico_wifi_cd;	/* WiFi virtual 'card detect' status */

static unsigned int pico_wifi_status(struct device *dev)
{
	return pico_wifi_cd;
}
...
int pico_wifi_set_carddetect(int val)
{
...
EXPORT_SYMBOL(pico_wifi_set_carddetect);
...
int __init pico_init_mmc(unsigned int sys_rev)
{
	uint32_t id;
	printk(KERN_INFO "%s\n", __func__);
	...
	id = GPIO_CFG(PICO_GPIO_WIFI_SHUTDOWN_N, 0, GPIO_CFG_OUTPUT,
		GPIO_CFG_NO_PULL, GPIO_CFG_2MA);
	gpio_tlmm_config(id, 0);
	gpio_set_value(PICO_GPIO_WIFI_SHUTDOWN_N, 0);

	/* wifi is SDCC2*/
	msm_add_sdcc(2, &pico_wifi_data);
	return 0;
}
