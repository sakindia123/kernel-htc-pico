...
unsigned int Msm7x27_TURBO_PERFLOCK_SCREEN_ON_MAX = 0;
...
#endif
static struct kernel_param_ops debug_mask_ops = {
	.set = param_set_int,
	.get = param_get_int,
};
module_param_cb(debug_mask, &debug_mask_ops, &debug_mask, S_IWUSR | S_IRUGO);
			       ...
			       unsigned long event, void *data)
{
	...
	if (event == CPUFREQ_START)
		...
		if (screen_on_policy_req) {
			...
			policy_max = Msm7x27_TURBO_PERFLOCK_SCREEN_ON_MAX;
			screen_on_policy_req--;
...
void __init perflock_init(struct perflock_platform_data *pdata)
{
	...
	if ((SOCINFO_VERSION_MAJOR(socinfo_get_version()) > 1)
		...
		&& (SOCINFO_VERSION_MINOR(socinfo_get_version()) >= 3))) {
			Msm7x27_TURBO_PERFLOCK_SCREEN_ON_MAX = 800000;
	} else {
		Msm7x27_TURBO_PERFLOCK_SCREEN_ON_MAX = 600000;
	}
