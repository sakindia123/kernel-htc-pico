...
if (smd_rpcrouter_debug_mask & RPC_MSG) \
	printk(KERN_ERR "[SMD] [RPC] "x); \
} while (0)

#define NTFY(x...) do { \
if (smd_rpcrouter_debug_mask & NTFY_MSG) \
	printk(KERN_ERR "[SMD] [NOTIFY] "x); \
} while (0)
#else
...
static void smemlog_debug_rpcrouter(uint32_t data1, uint32_t data2, uint32_t data3)
{
	smem_log_event(SMEM_LOG_PROC_ID_APPS |
			   RPC_ROUTER_LOG_EVENT_DEBUG_RPCROUTER,
			   data1,
			   data2,
			   data3);
}
				      ...
				      union rr_control_msg *msg)
{
	...
	if (!(msg->cmd == RPCROUTER_CTRL_CMD_HELLO) &&
		...
		printk(KERN_ERR "[SMD] rpcrouter_send_control_msg(): Warning, "
	...
	smemlog_debug_rpcrouter(1, HEADER, hdr.src_cid);
	xprt_info->xprt->write(&hdr, sizeof(hdr), HEADER);
	smemlog_debug_rpcrouter(2, PAYLOAD, hdr.size);
...
static void rpcrouter_register_board_dev(struct rr_server *server)
{
	...
	list_for_each_entry(board_info, &rpc_board_dev_list, list) {
		if (server->prog == board_info->dev->prog) {
			...
			if (rc)
				pr_err("[SMD] %s: board dev register failed %d\n",
				       __func__, rc);
			kfree(board_info);
			break;
		}
			       ...
			       union rr_control_msg *msg, int len)
{
	...
	case RPCROUTER_CTRL_CMD_RESUME_TX:
		...
		if (!r_ept) {
			printk(KERN_ERR
			       "[SMD] rpcrouter: Unable to resume client\n");
			break;
		}
		spin_lock_irqsave(&r_ept->quota_lock, flags);
		r_ept->tx_quota_cntr = 0;
	...
	case RPCROUTER_CTRL_CMD_NEW_SERVER:
		/* HTC Ryan.CL marked for enabling HTC RPC Functions
		if (msg->srv.vers == 0) {
			pr_err(
			"[SMD] rpcrouter: Server create rejected, version = 0, "
			"program = %08x\n", msg->srv.prog);
			break;
		}
		*/
	...
	case RPCROUTER_CTRL_CMD_REMOVE_CLIENT:
		...
		if (msg->cli.pid == RPCROUTER_PID_LOCAL) {
			printk(KERN_ERR
			       "[SMD] rpcrouter: Denying remote removal of "
			       "local client\n");
			break;
		}
		r_ept = rpcrouter_lookup_remote_endpoint(msg->cli.pid,
		...
		printk(KERN_ERR "[SMD] rpcrouter: LOCAL NOTIFICATION NOT IMP\n");
		rc = -ENOSYS;
...
static void *rr_malloc(unsigned sz)
{
	...
	printk(KERN_ERR "[SMD] rpcrouter: kmalloc of %d failed, retrying...\n", sz);
	do {
...
static void do_read_data(struct work_struct *work)
{
	...
	smemlog_debug_rpcrouter(3, HEADER, sizeof(hdr));
	...
	if (hdr.dst_cid == RPCROUTER_ROUTER_ADDRESS) {
		...
		smemlog_debug_rpcrouter(4, PAYLOAD, hdr.size);
	...
	smemlog_debug_rpcrouter(5, PACKMARK, pm);
	...
	smemlog_debug_rpcrouter(6, PAYLOAD, hdr.size);
	...
	)
{
	...
	smemlog_debug_rpcrouter(7, HEADER, hdr->src_cid);
	...
	smemlog_debug_rpcrouter(8, PACKMARK, pacmark);
	...
	smemlog_debug_rpcrouter(9, PAYLOAD, count);
...
static struct msm_rpc_reply *get_avail_reply(struct msm_rpc_endpoint *ept)
{
	...
	if (list_empty(&ept->reply_avail_q)) {
		if (ept->reply_cnt >= RPCROUTER_PEND_REPLIES_MAX) {
			printk(KERN_ERR
			       "[SMD] exceeding max replies of %d \n",
...
int msm_rpc_write(struct msm_rpc_endpoint *ept, void *buffer, int count)
{
	...
	if (count < (sizeof(uint32_t) * 2)) {
		printk(KERN_ERR "[SMD] rr_write: rejecting runt packet\n");
		return -EINVAL;
	}
	...
	if (rq->type == 0) {
		...
		if (count < (sizeof(uint32_t) * 6)) {
			printk(KERN_ERR
			       "[SMD] rr_write: rejecting runt call packet\n");
			return -EINVAL;
		}
		if (ept->dst_pid == 0xffffffff) {
			printk(KERN_ERR "[SMD] rr_write: not connected\n");
			return -ENOTCONN;
		}
		if ((ept->dst_prog != rq->prog) ||
			...
			printk(KERN_ERR
			       "[SMD] rr_write: cannot write to %08x:%08x "
			       "(bound to %08x:%08x)\n",
		...
		if (!reply) {
			printk(KERN_ERR
			       "[SMD] rr_write: rejecting, reply not found \n");
			return -EINVAL;
		}
	...
	if ((!r_ept) && (hdr.dst_pid != RPCROUTER_PID_LOCAL)) {
		printk(KERN_ERR
			"[SMD] msm_rpc_write(): No route to ept "
						  ...
						  unsigned flags)
{
	...
	for (;;) {
		...
		if (found_prog) {
			pr_info("[SMD] %s: server not found %x:%x\n",
				__func__, prog, vers);
			rc = -EHOSTUNREACH;
			break;
		}
