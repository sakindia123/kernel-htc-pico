...
static int msm_smd_debug_mask = MSM_SMD_INFO;
...
#define SMD_ERR(x...) printk(KERN_ERR "[SMD] "x)
#define SMSM_ERR(x...) printk(KERN_ERR "[SMD] "x)
...
void smd_ch(int ch)
{
	uint32_t *smd_ver;
	void *shared, *buffer;
	struct smd_half_channel *server, *remote;
	unsigned buffer_sz, found;
	...
	if (smd_ver && (((smd_ver[VERSION_MODEM] >> 16) >= 1) ||
		...
		shared = smem_find(ID_SMD_CHANNELS + ch,
			2 * sizeof(struct smd_half_channel));
		if (shared) {
			buffer = smem_get_entry(SMEM_SMD_FIFO_BASE_ID + ch,
				&buffer_sz);
			if (buffer)
				found = 1;
		}
		...
		shared = smem_find(ID_SMD_CHANNELS + ch,
			2 * (sizeof(struct smd_half_channel) + SMD_BUF_SIZE));
		if (shared) {
			found = 1;
			buffer_sz = SMD_BUF_SIZE;
		}
	}
	if (found) {
		server = shared;
		remote = shared + sizeof(struct smd_half_channel);
		pr_info("[SMD]ch%02d:"
			" %p|%8s(%04d/%04d) %c%c%c%c%c%c%c%c <->"
			" %p|%8s(%04d/%04d) %c%c%c%c%c%c%c%c : %5x\n", ch,
			server, chstate(server->state), server->tail, server->head,
			server->fDSR ? 'D' : 'd',
			server->fCTS ? 'C' : 'c',
			server->fCD ? 'C' : 'c',
			server->fRI ? 'I' : 'i',
			...
			server->fTAIL ? 'R' : 'r',
			server->fSTATE ? 'S' : 's',
			server->fBLOCKREADINTR ? 'B' : 'b',
			remote, chstate(remote->state), remote->tail, remote->head,
			remote->fDSR ? 'D' : 'd',
			remote->fCTS ? 'R' : 'r',
			remote->fCD ? 'C' : 'c',
			remote->fRI ? 'I' : 'i',
...
{
	SMD_ERR("MODEM/AMSS has CRASHED\n");
	smd_diag();
...
static void smd_channel_probe_worker(struct work_struct *work)
{
	...
	if (!shared) {
		SMD_ERR("%s: allocation table not initialized\n", __func__);
		return;
	}
...
static void smemlog_debug_smd(unsigned n, uint32_t data1, uint32_t data2, uint32_t data3)
{
	if (n == 2) {
		smem_log_event(SMEM_LOG_PROC_ID_APPS |
				   RPC_ROUTER_LOG_EVENT_DEBUG_SMD,
				   data1,
				   data2,
				   data3);
	}
...
static void handle_smd_irq(struct list_head *list, void (*notify)(void))
{
	...
	list_for_each_entry(ch, list, ch_list) {
		...
		if (ch_flags)
			smemlog_debug_smd(ch->n, 1, ch_flags, ch->recv->state);
		tmp = ch->recv->state;
		if (tmp != ch->last_state) {
			smd_state_change(ch, ch->last_state, tmp);
			state_change = 1;
		...
		if (state_change)
			smemlog_debug_smd(ch->n, 2, ch->last_state, state_change);
		if (ch_flags) {
			ch->update_state(ch);
				...
				int user_buf)
{
	...
	if (ch->n == 2)
		tmp1 = smd_stream_write_avail(ch);
	smemlog_debug_smd(ch->n, 3, 8191 - tmp1, ch->send->fHEAD);
	...
	if (ch->n == 2)
		tmp2 = smd_stream_write_avail(ch);
	smemlog_debug_smd(ch->n, 4, 8191 - tmp2, ch->send->fHEAD);
...
static int smd_stream_read(smd_channel_t *ch, void *data, int len, int user_buf)
{
	...
	if (ch->n == 2)
		tmp1 = smd_stream_read_avail(ch);
	smemlog_debug_smd(ch->n, 5, tmp1, ch->recv->fBLOCKREADINTR);

	r = ch_read(ch, data, len, user_buf);
	if (r > 0) {
		if (ch->n == 2)
			tmp2 = smd_stream_read_avail(ch);
		smemlog_debug_smd(ch->n, 6, tmp2, ch->recv->fBLOCKREADINTR);
...
static int smd_alloc_v1(struct smd_channel *ch)
{
	...
	if (!shared1) {
		SMD_ERR("smd_alloc_channel() cid %d does not exist\n", ch->n);
		return -1;
	}
	ch->send = &shared1->ch0;
	ch->recv = &shared1->ch1;
...
static int smd_alloc_channel(struct smd_alloc_elm *alloc_elm)
{
	...
	if (ch == 0) {
		SMD_ERR("smd_alloc_channel() out of memory\n");
		return -1;
	}
	ch->n = alloc_elm->cid;
	...
	SMD_INFO("smd_alloc_channel() cid=%02d size=%05d '%s' %s\n",
		    ch->n, ch->fifo_size, ch->name,
		    smd_is_packet(alloc_elm) ? "PACKET" : "STREAM");
...
int smd_write_start(smd_channel_t *ch, int len)
{
	...
	if (!ch) {
		SMD_ERR("%s: Invalid channel specified\n", __func__);
		return -ENODEV;
	}
	if (!ch->is_pkt_ch) {
		SMD_ERR("%s: non-packet channel specified\n", __func__);
		return -EACCES;
	}
	if (len < 1) {
		SMD_ERR("%s: invalid length: %d\n", __func__, len);
		return -EINVAL;
	}

	if (ch->pending_pkt_sz) {
		SMD_ERR("%s: packet of size: %d in progress\n", __func__,
			ch->pending_pkt_sz);
		return -EBUSY;
	}
	...
	if (ret < 0 || ret != sizeof(hdr)) {
		ch->pending_pkt_sz = 0;
		SMD_ERR("%s: packet header failed to write\n", __func__);
		return -EPERM;
	}
	return 0;
}
...
int smd_write_segment(smd_channel_t *ch, void *data, int len, int user_buf)
{
	...
	if (!ch->pending_pkt_sz) {
		SMD_ERR("%s: no transaction in progress\n", __func__);
		return -ENOEXEC;
	}
	if (ch->pending_pkt_sz - len < 0) {
		SMD_ERR("%s: segment of size: %d will make packet go over "
			"length\n", __func__, len);
		return -EINVAL;
	}
...
int smd_write_end(smd_channel_t *ch)
{
	...
	if (ch->pending_pkt_sz) {
		SMD_ERR("%s: current packet not completely written\n", __func__);
		return -E2BIG;
	}
...
static void *smem_alloc2(unsigned id, unsigned size_in)
{
	...
	if (!shared->heap_info.initialized) {
		SMD_ERR("%s: smem heap info not initialized\n", __func__);
		return NULL;
	}
	...
	if (toc[id].allocated) {
		...
		if (size_in != toc[id].size)
			SMD_ERR("%s: wrong size %u (expected %u)\n",
		...
		if (shared->heap_info.heap_remaining >= size_in) {
			...
			SMD_ERR("%s: not enough memory %u (required %u)\n",
...
void *smem_find(unsigned id, unsigned size_in)
{
	...
	size_in = ALIGN(size_in, 8);
	if (size_in != size) {
		SMD_ERR("smem_find(%d, %d): wrong size %d\n",
		       id, size_in, size);
		return 0;
	}
...
static int smsm_init(void)
{
	...
	if (i) {
		SMSM_ERR("%s: remote spinlock init failed %d\n", __func__, i);
		return i;
	}
	...
	if (!smsm_info.state) {
		...
		if (smsm_info.state) {
			...
			SMD_INFO("phy addr of smsm_info.state=0x%x\n",
			  ...
			  uint32_t clear_mask, uint32_t set_mask)
{
	...
	if (smsm_entry >= SMSM_NUM_ENTRIES) {
		SMSM_ERR("smsm_change_state: Invalid entry %d\n",
		       smsm_entry);
		return -EINVAL;
	}

	if (!smsm_info.intr_mask) {
		SMSM_ERR("smsm_change_intr_mask <SM NO STATE>\n");
		return -EIO;
	}
		      ...
		      uint32_t clear_mask, uint32_t set_mask)
{
	...
	if (!smsm_info.state) {
		SMSM_ERR("smsm_change_state <SM NO STATE>\n");
		return -EIO;
	}
	spin_lock_irqsave(&smem_lock, flags);
...
uint32_t smsm_get_state(uint32_t smsm_entry)
{
	...
	if (!smsm_info.state) {
		SMSM_ERR("smsm_get_state <SM NO STATE>\n");
	} else {
...
int smd_core_init(void)
{
	...
	if (r < 0)
		SMD_ERR("smd_core_init: enable_irq_wake "
...
static int __devinit msm_smd_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(channel_close_wq)) {
		SMD_ERR("%s: create_singlethread_workqueue ENOMEM\n", __func__);
		return -ENOMEM;
	}

	if (smsm_init()) {
		SMSM_ERR("smsm_init() failed\n");
		return -1;
	}

	if (smd_core_init()) {
		SMD_ERR("smd_core_init() failed\n");
		return -1;
	}
