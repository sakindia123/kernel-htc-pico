...
int diag_init_enabled_state;
#endif

/* the same table as drivers/usb/gadget/android.c */
enum {
	USB_FUNCTION_UMS = 0,
	USB_FUNCTION_ADB = 1,
	USB_FUNCTION_RNDIS,
...
int modem_to_userspace(void *buf, int r, int type, int is9k)
{
	...
	/*in this case, cmd may reply error message*/
		...
		DIAG_INFO("%s:check error cmd=0x%x message=ox%x\n",
			    __func__, value, *((uint8_t *)req->buf+1));
		if ((value == 0x27) || (value == 0x26)) {
			if (is9k == 1)
	...
	if (is9k == 1)
		print_hex_dump(KERN_DEBUG, "DM Read Packet Data from 9k radio "
			    "(first 16 Bytes): ", 16, 1, DUMP_PREFIX_ADDRESS,
			    req->buf, 16, 1);
	else
		print_hex_dump(KERN_DEBUG, "DM Read Packet Data from 7k radio "
			    "(first 16 Bytes): ", 16, 1, DUMP_PREFIX_ADDRESS,
			    req->buf, 16, 1);
	/* ctxt->rx_count += r; */
	req->actual = r;
...
static int vigor_diag_set(int enabled)
{
	...
	if (enabled) {
		...
		original_sum |=  (1 << USB_FUNCTION_DIAG);
		original_sum |=  (1 << USB_FUNCTION_DIAG_MDM);
		original_sum |=  (1 << USB_FUNCTION_RMNET);

		if (original_sum & (1 << USB_FUNCTION_MODEM))
			original_sum ^=  (1 << USB_FUNCTION_MODEM);
		if (original_sum & (1 << USB_FUNCTION_MODEM_MDM))
			original_sum ^=  (1 << USB_FUNCTION_MODEM_MDM);

	} else {
		if (original_sum & (1 << USB_FUNCTION_DIAG))
			original_sum ^=  (1 << USB_FUNCTION_DIAG);
		if (original_sum & (1 << USB_FUNCTION_DIAG_MDM))
			original_sum ^=  (1 << USB_FUNCTION_DIAG_MDM);
		if (original_sum & (1 << USB_FUNCTION_RMNET))
			original_sum ^=  (1 << USB_FUNCTION_RMNET);
	}
	...
	while (count > 0) {
		...
		case DM7K9K:
			DIAG_INFO("%s:DM7K9K sdio=%d\n", __func__,
				    sdio_diag_initialized);
			print_hex_dump(KERN_DEBUG, "DM Packet Data write to "
				    "radio ", 16, 1, DUMP_PREFIX_ADDRESS,
				    ctxt->toARM9_buf, writed, 1);
...
#if defined(CONFIG_MACH_VIGOR)
			if (driver->sdio_ch) {
				memcpy(diag2arm9_buf_9k, ctxt->toARM9_buf, writed);
		...
		case DM7K9KDIFF:
			...
			print_hex_dump(KERN_DEBUG, "DM Packet Data write to "
				    "radio ", 16, 1, DUMP_PREFIX_ADDRESS,
				    ctxt->toARM9_buf, writed, 1);
			if ((ctxt->toARM9_buf[3] & 0x80) == 0x80) {
				...
				if (sdio_diag_initialized) {
					...
					if (decode_encode_hdlc(ctxt->toARM9_buf, &writed,
						    buf_9k, 1, 3)) {
						kfree(buf_9k);
						mutex_unlock(&ctxt->diag2arm9_write_lock);
						return -EINVAL;
...
retry:
	...
	if (ctxt->read_arm9_count == 0) {
		print_hex_dump(KERN_DEBUG, "DM Packet Data read from "
			    "radio ", 16, 1, DUMP_PREFIX_ADDRESS,
...
/*	ctxt = kzalloc(sizeof(*ctxt), GFP_KERNEL);*/
	...
	DIAG_INFO("%s: ch->name:%s ctxt:%p pkts_pending:%p\n", __func__,
		    legacych->name, legacyctxt,
		    &legacyctxt->dpkts_tolaptop_pending);
#if defined(CONFIG_USB_ANDROID_LTE_DIAG)
	...
	DIAG_INFO("%s: ch->name:%s ctxt:%p pkts_pending:%p\n", __func__,
		    mdmch->name, mdmctxt,
		    &mdmctxt->dpkts_tolaptop_pending);
#endif
...
static void diag_function_disable(struct usb_function *f)
{
	...
	DIAG_INFO("%s: dev=%s\n", __func__,
		    (dev == mdmctxt) ? DIAG_MDM : DIAG_LEGACY);
