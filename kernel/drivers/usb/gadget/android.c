...
extern void init_mfg_serialno(void);
		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	if (((ctrl->bRequestType << 8) | ctrl->bRequest) ==
		...
		pr_debug("usb: get OS feature descriptor\n");
		if (w_index != 0x0004 || w_length > FEATURE_DESC_SIZE)
			return ret;

		product_id = get_product_id(dev);
		pr_info("mtp: product_id = 0x%x\n", product_id);
...
static inline bool CheckSwitchEvent(struct usb_function *f, int iFuncDisable)
{
	...
	/* send composite uevent when state changed */
		if (f && (f->dev)) {
			pr_debug("[USB] %s: hidden=%d,disable=%d\n",
...
int android_switch_function(unsigned func)
{
	...
	list_for_each_entry(f, &android_config_driver.functions, list) {
		...
		if (!strcmp(f->name, "usb_mass_storage")) {
			...
			bNeedConfigUevent |=
			    CheckSwitchEvent(f, (func & (1 << USB_FUNCTION_MTP)) ? 0 : 1);
	#endif
		/* also enable adb with MTP function */
			...
			bNeedConfigUevent |=
			    CheckSwitchEvent(f, (func & (1 << USB_FUNCTION_MTP36)) ? 0 : 1);
	#endif
		/* also enable adb with MTP function */
		else if ((func & (1 << USB_FUNCTION_MTP36)) &&
			!strcmp(f->name, "adb"))
	...
	if (bNeedConfigUevent)
		pr_debug("[USB] %s: Send usb_configuration uevent\n", __func__);
