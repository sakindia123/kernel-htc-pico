...
#define pr_debug(fmt, args...) \
	printk(KERN_DEBUG "[USBH] " pr_fmt(fmt), ## args)

#ifdef pr_err
#undef pr_err
#endif
#define pr_err(fmt, args...) \
	printk(KERN_ERR "[USBH] " pr_fmt(fmt), ## args)

#ifdef pr_info
#undef pr_info
#endif
#define pr_info(fmt, args...) \
	printk(KERN_INFO "[USBH] " pr_fmt(fmt), ## args)
...
{
	struct msm_otg *dev = the_msm_otg;
#ifdef CONFIG_USB_BYPASS_VBUS_NOTIFY
	pr_info("bypass battery vbus, otgsc_bsv:%d\n",
		    (OTGSC_BSV & readl(USB_OTGSC)) ? 1 : 0);
...
static int msm_otg_phy_reset(struct msm_otg *dev)
{
	...
	/* This will cause ID pin unnormal. ID pin interrupt will trigger
