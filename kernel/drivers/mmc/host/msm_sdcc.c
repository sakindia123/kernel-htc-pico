...
/* monitor dma access */
static ktime_t sd_dma_tick_s, sd_dma_tick_e;
...
int is_sd_platform(struct mmc_platform_data *plat)
{
...
{
	if (host->mmc->card && host->mmc->card->type == MMC_TYPE_SD) {
		sd_dma_tick_s = ktime_get();
	}
			  ...
			  struct msm_dmov_errdata *err)
{
	...
	if (host->mmc->card && host->mmc->card->type == MMC_TYPE_SD) {
		sd_dma_tick_e = ktime_sub(ktime_get(), sd_dma_tick_s);
		sd_dma_tick = (int) ktime_to_ms(sd_dma_tick_e);
		if (sd_dma_tick >= 3000)
			printk("SD_DMA complete time >= %d ms\n", sd_dma_tick);
	}
...
static void msmsdcc_req_tout_timer_hdlr(unsigned long data)
{
	...
	pr_info("%s: %s, pwr %d, clks_on %d, irq_disabled %d\n", mmc_hostname(host->mmc),
		__func__, host->pwr, host->clks_on, host->sdcc_irq_disabled);
	pr_info("%s: intmask0 0x%x, mciclock 0x%x\n", mmc_hostname(host->mmc),
		readl_relaxed(host->base + MMCIMASK0), readl_relaxed(host->base + MMCICLOCK));
	spin_lock_irqsave(&host->lock, flags);
	if ((host->plat->dummy52_required) &&
		(host->dummy_52_state == DUMMY_52_STATE_SENT)) {
...
static int msmsdcc_suspend(struct device *dev)
{
	...
	if (mmc) {
		...
		if (mmc->card && !is_wifi_platform(host->plat)) {
		/*
...
static int msmsdcc_resume(struct device *dev)
{
	...
	if (mmc) {
		...
		if ((mmc->card) && !is_wifi_platform(host->plat) && (!host->eject)) {
			mmc_resume_host(mmc);
		} else {
			spin_lock_irqsave(&host->lock, flags);
			msmsdcc_switch_clock(host->mmc, 1);
...
//HTC_WIFI_START
	perf_lock_init(&wlan_perf_lock, PERF_LOCK_HIGHEST, "bcm4330");
//HTC_WIFI_END
