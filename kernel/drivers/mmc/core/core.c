...
extern int is_wifi_platform(struct mmc_platform_data *plat);
extern int is_sd_platform(struct mmc_platform_data *plat);
//HTC_WIFI_END
...
// HTC_WIFI_END
	...
	case PM_SUSPEND_PREPARE:
		...
		/* SD card may be removed in suspend mode */
		if (is_sd_platform(sdcc_host->plat))
			host->rescan_disable = 0;
		else
	...
	case PM_POST_RESTORE:
		...
		spin_unlock_irqrestore(&host->lock, flags);
		// HTC_WIFI_START
		if (!is_wifi_platform(sdcc_host->plat))
			mmc_detect_change(host, 0);
		// HTC_WIFI_END
	}
