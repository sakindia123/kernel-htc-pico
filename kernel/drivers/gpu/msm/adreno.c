...
static int adreno_start(struct kgsl_device *device, unsigned int init_ram)
{
	...
	*/
	...
	adreno_regread(device, REG_RBBM_WAIT_IDLE_CLOCKS, &idle_clocks);
	KGSL_DRV_ERR(device, "WAIT_IDLE_CLOCKS was 0x%08X\n", idle_clocks);
	idle_clocks = 0xFF;
	adreno_regwrite(device, REG_RBBM_WAIT_IDLE_CLOCKS, idle_clocks);
	KGSL_DRV_ERR(device, "WAIT_IDLE_CLOCKS is now 0x%08X\n", idle_clocks);
				...
				unsigned int msecs)
{
	...
	if (!kgsl_check_timestamp(device, timestamp)) {
		...
		if (status <= 0)
			KGSL_DRV_WARN(device, "kgsl_wait_io_event_interruptible_timeout return %ld\n", status);
		mutex_lock(&device->mutex);
