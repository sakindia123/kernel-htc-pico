...
static int
QUP_i2c_recover_bus_busy_sp1(struct qup_i2c_dev *dev, struct htc_i2c_platform_data *pdata)
{
	int i;
	int gpio_clk = pdata->gpio_clk;
	int gpio_dat = pdata->gpio_data;
	bool gpio_clk_status = false;
	int clk_value = 0;
	int dat_value = 0;

	printk(KERN_DEBUG "%s: try to recover..\n", __func__);

	gpio_tlmm_config(pdata->recoverSetting[0], GPIO_CFG_ENABLE);
	gpio_tlmm_config(pdata->recoverSetting[1], GPIO_CFG_ENABLE);

	clk_value = gpio_get_value(gpio_clk);
	dat_value = gpio_get_value(gpio_dat);

	printk(KERN_INFO "%s: dev->adapter.nr: %d, gpio_clk: %d, gpio_dat: %d\n",
			__func__, dev->adapter.nr, gpio_clk, gpio_dat);

	dev_info(dev->dev, "Before recover, QUP clk_value = %d, dat_value = %d\n",
			clk_value, dat_value);

	if ((clk_value == 0) || (dat_value == 0)) {
		dev_err(dev->dev, "QUP_i2c_err, try to recover\n");

		printk(KERN_DEBUG "i2c_scl: %d, i2c_sda: %d\n",
	...
	gpio_tlmm_config(pdata->operatingSetting[0], GPIO_CFG_ENABLE);
	gpio_tlmm_config(pdata->operatingSetting[1], GPIO_CFG_ENABLE);

	dev_info(dev->dev, "After recover, QUP clk_value = %d, dat_value = %d\n",
			clk_value, dat_value);

	if ((clk_value == 0) || (dat_value == 0))
		printk(KERN_DEBUG "Bus still busy\n");
	else {
		printk(KERN_DEBUG "Bus busy cleared after %d clock cycles\n", i);
		return 0;
	}
...
static int
QUP_i2c_recover_bus_busy(struct qup_i2c_dev *dev, struct htc_i2c_platform_data *pdata)
{
	int i;
	uint32_t status = readl_relaxed(dev->base + QUP_I2C_STATUS);
	int gpio_clk = pdata->gpio_clk;
	...
	printk(KERN_DEBUG "QUP_i2c_recover_bus_busy, QUP_I2C_STATUS = 0x%x\n", status);

	if (!(status & (I2C_STATUS_BUS_ACTIVE)) || (status & (I2C_STATUS_BUS_MASTER)))
		return 0;


	printk(KERN_DEBUG "QUP_i2c_recover_bus_busy, try to recover..\n");

	gpio_tlmm_config(pdata->recoverSetting[0], GPIO_CFG_ENABLE);
	gpio_tlmm_config(pdata->recoverSetting[1], GPIO_CFG_ENABLE);

	printk(KERN_INFO "QUP_i2c_recover_bus_busy, dev->adapter.nr: %d, gpio_clk: %d, gpio_dat: %d\n", dev->adapter.nr, gpio_clk, gpio_dat);
	...
	printk(KERN_DEBUG "Bus still busy, status %x\n",
		 status);
		...
		do {
...
#ifdef CONFIG_I2C_HTCTOOLS
				if (dev->pdata->htc_pdata) {
					if (dev->pdata->htc_pdata->allocDone) {
						disable_irq(dev->err_irq);
						ret = QUP_i2c_recover_bus_busy(dev, dev->pdata->htc_pdata);
						if (ret)
							printk(KERN_ERR "[QUP I2C Err] QUP_i2c_recover_bus_busy: ret = %d\n", ret);
						enable_irq(dev->err_irq);
					}
				}
#endif
			...
			if (dev->err) {
				if (dev->err > 0 &&
					...
					dev_info(dev->dev, "QUP I2C ELSE case\n");
					if (dev->err > 0) {
						disable_irq(dev->err_irq);
						ret = QUP_i2c_recover_bus_busy_sp1(dev, dev->pdata->htc_pdata);
						if (ret) {
							printk(KERN_ERR "[QUP I2C Err]"
								" QUP_i2c_recover_bus_busy_sp1: "
								"ret = %d\n", ret);
						}
						enable_irq(dev->err_irq);
					}
...
#ifdef CONFIG_I2C_HTCTOOLS
	pdata->setupHtcPlatformData(dev->adapter.nr, pdata);
#endif
...
#ifdef CONFIG_I2C_HTCTOOLS
	dev->pdata->freeHtcPlatformData(dev->pdata);
#endif
	del_timer_sync(&dev->pwr_timer);
	if (dev->clk_state != 0)
		qup_i2c_pwr_mgmt(dev, 0);
