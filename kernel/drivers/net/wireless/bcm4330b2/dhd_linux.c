...
int wifi_get_cscan_enable(void)
{
        ...
        return 0;
#else
        //Always use combo scan for bcm4330 driver
        return 1;
#endif
}
...
#endif /* defined (CUSTOMER_HW2) && (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 35)) */
...
dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	// HTC_WIFI_START
	/* For power consumption and we will not handle this event, we remove this mask*/
...
int dhd_os_wake_lock_timeout(dhd_pub_t *pub)
{
	...
	if (dhd) {
		...
		if (dhd->wakelock_timeout_enable)
			wake_lock_timeout(&dhd->wl_rxwake, 25);
#endif
		dhd->wakelock_timeout_enable = 0;
