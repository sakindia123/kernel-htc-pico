...
void dhd_suspend_pktfilter(dhd_pub_t *dhd, int suspend)
{
	...
	printk(KERN_INFO "Enter set packet filter in %s\n", suspend?"suspend":"resume");
		...
		for (i = 0; i < DEFAULT_MAX_NUM_FILTERS; i++) {
			...
			if (pkt_id) {
				if (pkt_id < 200) {
					...
					dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, buf, sizeof(buf), WL_IOCTL_ACTION_SET);
				} else {
					/* enable it!! */
					enable_parm.id = htod32(pkt_id);
...
/* packet filter for Rogers nat keep alive +++ */
		...
		if (i == DEFAULT_MAX_NUM_FILTERS) {
			...
			if (empty_found)
				...
				printk(KERN_INFO "no enough room for filter %d!!\n", pkt_id);
		}
	} else {
		enable_parm.id = htod32(pkt_id);
		enable_parm.enable = htod32(1);
