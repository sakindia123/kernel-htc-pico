...
void diagfwd_debug_dump(void)
{
	int i;

	printk("Diag_buf_idx:%d diag_buf_follows\n", diag_debug_buf_idx);
	for (i = 0; i < 1024; i++)
		printk("%c", diag_debug_buf[i]);
}
EXPORT_SYMBOL(diagfwd_debug_dump);
			...
			if (!buf)
				...
				case 2:
					print_hex_dump(KERN_DEBUG, "Read Packet Data "
						    ...
						    DUMP_PREFIX_ADDRESS, buf, 16, 1);
					print_hex_dump(KERN_DEBUG, "Read Packet Data "
						    "from 7K(last 16 bytes) ", 16, 1,
						    DUMP_PREFIX_ADDRESS, buf+r-16, 16, 1);
					break;

				default:
					print_hex_dump(KERN_DEBUG, "Read Packet Data "
						    "from 7K ", 16, 1, DUMP_PREFIX_ADDRESS,
						    buf, r, 1);
				}
#if DIAG_XPST
...
#ifdef SDQXDM_DEBUG
				if (full) {
					pr_err("[diag-dbg] buffer become available %d %d, "
						    "read %d\n", driver->in_busy_1,
						    driver->in_busy_2, r);
					full = 0;
				}
				...
				if (diff > 1000) {
					pr_err("[diag-dbg] Over time (%ld) %ld.%04ld "
						    "-> %ld.%04ld empty = %d\n", diff,
						    (long)t0.tv_sec, t0.tv_usec/1000,
			...
			if (!buf)
				...
				case 2:
					print_hex_dump(KERN_DEBUG, "Read Packet Data "
						    ...
						    DUMP_PREFIX_ADDRESS, buf, 16, 1);
					print_hex_dump(KERN_DEBUG, "Read Packet Data "
						    "from qdsp(last 16 bytes) ", 16, 1,
						    DUMP_PREFIX_ADDRESS, buf+r-16, 16, 1);
					break;
				default:
					print_hex_dump(KERN_DEBUG, "Read Packet Data "
						    "from qdsp ", 16, 1, DUMP_PREFIX_ADDRESS,
						    buf, r, 1);
				}
#if  DIAG_XPST
				type = checkcmd_modem_epst(buf);
...
void diag_process_hdlc(void *data, unsigned len)
{
	...
	if ((driver->ch) && (ret) && (type) && (hdlc.dest_idx > 3)) {
		...
		if (diag7k_debug_mask)
			print_hex_dump(KERN_DEBUG, "Written Packet Data to SMD: ",
...
#else
		r = smd_open("DSP_DIAG", &driver->chqdsp, driver,
