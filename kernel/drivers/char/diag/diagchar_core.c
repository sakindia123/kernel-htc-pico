...
static
ssize_t diag_dump(struct device *dev, struct device_attribute *attr, char *buf)
{
	uint16_t i, p;
	...
	print_hex_dump(KERN_INFO, "diag_debug_buf:", 16, 1, DUMP_PREFIX_ADDRESS,
		    diag_debug_buf, 1026, 1);
			   ...
			   unsigned int iocmd, unsigned long ioarg)
{
	...
	if (iocmd == DIAG_IOCTL_SWITCH_LOGGING) {
		...
		if (driver->logging_mode == MEMORY_DEVICE_MODE) {
			...
			/* Poll SMD channels to check for data*/
			queue_work(driver->mdm_diag_workqueue,
				    &(driver->diag_read_smd_mdm_work));
#endif
#if defined(CONFIG_ARCH_MSM8X60_LTE)
			driver->in_busy_sdio = 0;
			buf_9k = kzalloc(USB_MAX_OUT_BUF, GFP_KERNEL);
